...
-(void)viewDidLoad{
    ...
    //扫描区域
    CGRect areaRect = CGRectMake((screen_width - 218)/2, (screen_height - 218 - 64)/2, 218, 218);
...
-(void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection{
    if (metadataObjects.count>0) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(didScanText:)]) {
            [self.delegate didScanText:metadataObject.stringValue];
        }
