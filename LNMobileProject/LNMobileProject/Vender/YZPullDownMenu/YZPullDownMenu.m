...
- (void)setup
{
    ...
    _observer = [[NSNotificationCenter defaultCenter] addObserverForName:YZUpdateMenuTitleNote object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification * _Nonnull note) {
        ...
        if (col >= 0 && col < self.menuButtons.count) {
...
- (void)layoutSubviews
{
    ...
    for (NSInteger i = 0; i < count; i++) {
        ...
        if (i < count - 1) {
            ...
            separateLine.frame = CGRectMake(
                                            CGRectGetMaxX(btn.frame),
                                            _separateLineTopMargin,
                                            1,
                                            btnH - 2 * _separateLineTopMargin);
        }
    }
    
    // 设置底部View位置
    CGFloat bottomH = SINGLE_LINE_WIDTH;
...
- (void)reload
{
    ...
    for (NSInteger col = 0; col < cols; col++) {
        ...
        UIImageView *imgView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"screen_icon_down"]];
        ...
        [imgView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(menuButton.titleLabel.mas_right).offset(4);
            make.centerY.equalTo(menuButton.mas_centerY);
        }];
...
- (void)reloadColumn:(NSInteger)col {
    // 获取按钮
    UIView *view = [self viewWithTag:col];
    ...
    // 添加按钮
    [self.menuButtons setObject:menuButton atIndexedSubscript:col];
    ...
    CGFloat height = [self.dataSource pullDownMenu:self heightForColAtIndex:col];
    //[self.colsHeight addObject:@(height)];
    [self.colsHeight setObject:@(height) atIndexedSubscript:col];
    
    // 保存所有子控制器
    UIViewController *vc = [self.dataSource pullDownMenu:self viewControllerForColAtIndex:col];
    [self.controllers setObject:vc atIndexedSubscript:col];
...
- (void)dismiss
{
    ...
    for (UIButton *button in self.menuButtons) {
        if (button.selected) {
            button.selected = NO;
            UIImageView *imgView = self.imgViews[button.tag];
            [imgView setImage:[UIImage imageNamed:@"screen_icon_down"]];
        }
...
- (void)btnClick:(UIButton *)button
{
    ...
    if (button.tag == 1) {
        //点击了属性  看看属性有没有加载，如果没有，提示属性没加载
        if (!self.isAttriLoaded) {
    ...
    for (UIButton *otherButton in self.menuButtons) {
        ...
        if (otherButton.selected) {
            ...
            UIImageView *imgView = self.imgViews[otherButton.tag];
            [imgView setImage:[UIImage imageNamed:@"screen_icon_down"]];
        }
    ...
    if (button.selected == YES) { // 当按钮选中，弹出蒙版
        ...
        [imgView setImage:[UIImage imageNamed:@"screen_icon_up_n"]];
