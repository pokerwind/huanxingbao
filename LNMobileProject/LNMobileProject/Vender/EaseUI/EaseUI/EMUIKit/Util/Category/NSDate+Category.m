...
//    13:昨天HH:mm
    if (!hasAMPM) { //24-hour clock
        ...
        if (hour >= 0 && hour <= 6) {
            ...
            NSString *format = @"下午hh:mm";
            dateFormatter = [NSDateFormatter dateFormatterWithFormat:format];
            NSString *fmt = dateFormatter.dateFormat;
            NSLog(@"format:%@,fmt:%@",format,fmt);
...
//        }else if (hour > 11 && hour <= 17) {
//            NSString *format = NSEaseLocalizedString(@"NSDateCategory.text11", @"");
//            dateFormatter = [NSDateFormatter dateFormatterWithFormat:format];
//            NSString *fmt = dateFormatter.dateFormat;
//            NSLog(@"format:%@,fmt:%@",format,fmt);
//        }else if (hour > 17 && hour <= 24) {
