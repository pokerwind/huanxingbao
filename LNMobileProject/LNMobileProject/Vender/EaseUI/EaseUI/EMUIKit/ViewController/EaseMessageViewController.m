...
- (void)viewDidLoad {
    ...
    EMChatToolbarType barType = self.conversation.type == 0 ? 0 : 1;
    ...
    UIImage *image = [UIImage easeImageNamed:@"EaseUIResource.bundle/chat_sender_audio_playing_full"];
...
- (void)_downloadMessageAttachments:(EMMessage *)message
{
    ...
    if ([messageBody type] == 2) {
        EMImageMessageBody *imageBody = (EMImageMessageBody *)messageBody;
        if (imageBody.thumbnailDownloadStatus > 1)
        {
    ...
    else if ([messageBody type] == 3)
    {
        EMVideoMessageBody *videoBody = (EMVideoMessageBody *)messageBody;
        if (videoBody.thumbnailDownloadStatus > 1)
        {
            //download the message thumbnail
            [[[EMClient sharedClient] chatManager] downloadMessageThumbnail:message progress:nil completion:completion];
        }
    }
    else if ([messageBody type] == 5)
    {
        EMVoiceMessageBody *voiceBody = (EMVoiceMessageBody*)messageBody;
        if (voiceBody.downloadStatus > 1)
        {
                                   ...
                                   read:(BOOL)read
{
    ...
    if (message.chatType != 0 || message.isReadAcked || [account isEqualToString:message.from] || ([UIApplication sharedApplication].applicationState == UIApplicationStateBackground) || !self.isViewDidAppear)
    {
        return NO;
    }
...
- (void)_videoMessageCellSelected:(id<IMessageModel>)model
{
    ...
    if (videoBody.thumbnailDownloadStatus == 2 || ![[NSFileManager defaultManager] fileExistsAtPath:videoBody.thumbnailLocalPath]) {
    ...
    if (videoBody.downloadStatus == 1 && [[NSFileManager defaultManager] fileExistsAtPath:localPath])
    {
        block();
        return;
    }
...
{
    __weak typeof(self) weakSelf = self;
    void (^refresh)(NSArray *messages) = ^(NSArray *arr) {
        NSMutableArray *messages = [[NSMutableArray alloc] initWithArray:arr];
        if (self.isFromProductDetail) {
            NSString *serviceMessage = @"你好";
            EMTextMessageBody *body = [[EMTextMessageBody alloc] initWithText:serviceMessage];
            NSString *from = self.conversation.conversationId;
            // 生成Message
            EMMessage *message = [[EMMessage alloc] initWithConversationID:self.conversation.conversationId from:from to:[[EMClient sharedClient] currentUsername] body:body ext:@{@"type":@"CommodityLike"}];
        ...
        dispatch_async(_messageQueue, ^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (strongSelf) {
                    ...
                    NSInteger index = [self.dataArray count] - scrollToIndex - 1;
                    if (index <= 0) {
                        index = 0;
                    }
                    if (index >0) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        if ([[model.message.ext objectForKey:@"type"] isEqualToString:@"CommodityLike"]) {
            
            CommodityLikeCell *cell = [[[NSBundle mainBundle] loadNibNamed:@"CommodityLikeCell" owner:nil options:nil] lastObject];
            [cell.commodityImage sd_setImageWithURL:[NSURL URLWithString:self.commodityImage]];
            cell.commodityName.text = self.commodityName;
            cell.commodityPrice.text = [NSString stringWithFormat:@"￥%@",self.commodityPrice];
            ...
            [[[cell.sendBtn rac_signalForControlEvents:UIControlEventTouchUpInside] takeUntil:cell.rac_prepareForReuseSignal] subscribeNext:^(id x) {
                //生成Message
                NSString *link = DEFAULT_HTTP_HOST.a(@"/goods-").a(self.commodityID);
                EMTextMessageBody *body = [[EMTextMessageBody alloc] initWithText:link];
                NSString *from = [[EMClient sharedClient] currentUsername];
                EMMessage *message = [[EMMessage alloc] initWithConversationID:self.conversation.conversationId from:from to:self.conversation.conversationId body:body ext:@{@"type":@"CommodityInfo",@"commodityName":self.commodityName,@"commodityImage":self.commodityImage,@"commodityPrice":self.commodityPrice,@"commodityID":self.commodityID}];
                [self _sendMessage:message];
                
//                EMTextMessageBody *body2 = [[EMTextMessageBody alloc] initWithText:@"测试文本"];
//                EMMessage *message2 = [[EMMessage alloc] initWithConversationID:self.conversation.conversationId from:from to:self.conversation.conversationId body:body2 ext:nil];
//                
//                [self _sendMessage:message2];
                
            }];
            
            return cell;
        }
        if ([[model.message.ext objectForKey:@"type"] isEqualToString:@"CommodityInfo"]) {
            
            // 发送方
            NSDictionary *dic = model.message.ext;
            if ([model.message.from isEqualToString:[DPMobileApplication sharedInstance].loginUser.emchat_username]) {
                EaseCustomSendMessageCell *cell = [[[NSBundle mainBundle] loadNibNamed:@"EaseCustomSendMessageCell" owner:nil options:nil] lastObject];
                [cell.headImage sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",model.avatarURLPath]] placeholderImage:model.avatarImage];
                cell.nicknameLabel.text = model.nickname;
                [cell.shareImage sd_setImageWithURL:[NSURL URLWithString:dic[@"commodityImage"]]];
                cell.titleLabel.text = dic[@"commodityName"];
                cell.detailLabel.text =@"￥".a(dic[@"commodityPrice"]);
                [[[cell.selectBtn rac_signalForControlEvents:UIControlEventTouchUpInside] takeUntil:cell.rac_prepareForReuseSignal] subscribeNext:^(id x) {
                    ...
                    goodsDetailVC.goodsId = [NSString stringWithFormat:@"%@",dic[@"commodityID"]];
            ...
            // 接收方
                ...
                EaseCustomReceiveMessageCell *cell = [[[NSBundle mainBundle] loadNibNamed:@"EaseCustomReceiveMessageCell" owner:nil options:nil] lastObject];
                [cell.headImage sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",model.avatarURLPath]] placeholderImage:model.avatarImage];
                cell.nicknameLabel.text = model.nickname;
                [cell.shareImage sd_setImageWithURL:[NSURL URLWithString:dic[@"commodityImage"]]];
                cell.titleLabel.text = dic[@"commodityName"];
                ...
                [[[cell.selectBtn rac_signalForControlEvents:UIControlEventTouchUpInside] takeUntil:cell.rac_prepareForReuseSignal] subscribeNext:^(id x) {
                    ...
                    [self.navigationController pushViewController:goodsDetailVC animated:YES];
                    // 产品详情->网页
                    //NSString *format = [SPCommonUtils getFullUrlWithHost:DEFAULT_HTTP_HOST partUrl:@"index.php/Mobile/Goods/goodsInfo?id=%@"];
//                    NSString *format = FULL_URL(@"index.php/Mobile/Goods/goodsInfo?id=%@");
//                    YCWebViewController* webVC = [[YCWebViewController alloc]init];
//                    webVC.title = @"产品详情";
//                    webVC.web_url = [NSString stringWithFormat:format,dic[@"commodityID"]];
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(messageViewController:cellForMessageModel:)]) {
            ...
            if (cell) {
                ...
                if ([cell isKindOfClass:[EaseMessageCell class]]) {
                    ...
                    emcell.avatarView.clipsToBounds = YES;
...
{
    if ((model.messageStatus != 3) && (model.messageStatus != 0))
    {
        return;
    }
...
{
    if ([self.delegate respondsToSelector:@selector(messageViewController:selectAtTarget:)] && self.conversation.type == 1) {
        location += 1;
        __weak typeof(self) weakSelf = self;
...
- (void)didMessageAttachmentsStatusChanged:(EMMessage *)message
    ...
    if (!error) {
        ...
        }else if([fileBody type] == 3){
            EMVideoMessageBody *videoBody = (EMVideoMessageBody *)fileBody;
            if ([videoBody thumbnailDownloadStatus] == 1)
            {
                [self _reloadTableViewDataWithMessage:message];
            }
        }else if([fileBody type] == 5){
            if ([fileBody downloadStatus] == 1)
            {
                [self _reloadTableViewDataWithMessage:message];
            }
        }
...
- (void)_sendMessage:(EMMessage *)message
{
    ...
    NSString *nick = [DPMobileApplication sharedInstance].loginUser.nickname;
    NSString *avatar = [DPMobileApplication sharedInstance].loginUser.head_pic;
    
    NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithDictionary:@{@"nick":@"昵称未设置",@"avatar":@""}];
    if (nick) {
    ...
    [dict addEntriesFromDictionary:message.ext];
    message.ext = dict;
...
- (void)sendImageMessageWithData:(NSData *)imageData
{
    ...
    if (_dataSource && [_dataSource respondsToSelector:@selector(messageViewController:progressDelegateForMessageBodyType:)]) {
        progress = [_dataSource messageViewController:self progressDelegateForMessageBodyType:2];
    }
    else{
        progress = self;
    }
                             ...
                             duration:(NSInteger)duration
{
    id progress = nil;
    if (_dataSource && [_dataSource respondsToSelector:@selector(messageViewController:progressDelegateForMessageBodyType:)]) {
        progress = [_dataSource messageViewController:self progressDelegateForMessageBodyType:5];
    }
    else{
        progress = self;
    }
...
- (void)sendVideoMessageWithURL:(NSURL *)url
{
    id progress = nil;
    if (_dataSource && [_dataSource respondsToSelector:@selector(messageViewController:progressDelegateForMessageBodyType:)]) {
        progress = [_dataSource messageViewController:self progressDelegateForMessageBodyType:3];
    }
    else{
        progress = self;
    }
