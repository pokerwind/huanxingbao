...
@interface DZWPInfoVC ()<UIImagePickerControllerDelegate, UINavigationControllerDelegate, UIActionSheetDelegate>
...
- (IBAction)submitButtonClick {
    ...
    if (!self.addressTextField.text.length) {
        [SVProgressHUD showInfoWithStatus:@"请输入店铺详细地址"];
        return;
    }
    ...
    NSDictionary *params = @{@"contacts_name":self.contactTextField.text, @"user_mobile":self.mobileTextField.text, @"province":self.province, @"city":self.city, @"address":self.addressTextField.text, @"category_id":self.categoryId, @"retail_amount":self.handCountTextField.text, @"basic_amount":self.expressCountTextField.text, @"allow_return":self.supportSwitch.on?@"1":@"0", @"description":[NSString stringWithFormat:@"%@", self.infoTextView.text]};
...
- (void)getData{
    ...
    [api startWithBlockSuccess:^(__kindof LCBaseRequest *request) {
        ...
        if (self.model.isSuccess) {
            ...
            self.addressTextField.text = self.model.data.address;
