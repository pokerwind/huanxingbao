...
@interface DZSTInfoVC ()<UIImagePickerControllerDelegate, UINavigationControllerDelegate, UIActionSheetDelegate>
...
- (IBAction)submitButtonClick {
    if (!self.marketTextField.text.length) {
        [SVProgressHUD showInfoWithStatus:@"请输入批发市场"];
        return;
    }
    if (!self.floorTextField.text.length) {
        [SVProgressHUD showInfoWithStatus:@"请输入所在楼层"];
        return;
    }
    if (!self.numberTextField.text.length) {
        [SVProgressHUD showInfoWithStatus:@"请输入档口编号"];
        return;
    }
    ...
    NSDictionary *params = @{@"contacts_name":self.contactTextField.text, @"user_mobile":self.mobileTextField.text, @"province":self.province, @"city":self.city, @"market_name":self.marketTextField.text,@"floor_number":self.floorTextField.text, @"room_number":self.numberTextField.text, @"category_id":self.categoryId, @"retail_amount":self.handCountTextField.text, @"basic_amount":self.expressCountTextField.text, @"allow_return":self.supportSwitch.on?@"1":@"0", @"description":[NSString stringWithFormat:@"%@", self.infoTextView.text]};
...
- (void)getData{
    ...
    [api startWithBlockSuccess:^(__kindof LCBaseRequest *request) {
        ...
        if (self.model.isSuccess) {
            ...
            [self.coverImageView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@", DEFAULT_HTTP_IMG, self.model.data.shop_real_pic]]];
            ...
            self.marketTextField.text = self.model.data.market_name;
            self.floorTextField.text = self.model.data.floor_number;
            self.numberTextField.text = self.model.data.room_number;
            ...
            self.licenseNameLabel.text = self.model.data.company_name;
            self.licenseNumberLabel.text = self.model.data.reg_number;
            if (self.model.data.license.length) {
                [self.licenseImageView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@", DEFAULT_HTTP_IMG, self.model.data.license]]];
            }
