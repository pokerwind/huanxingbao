...
@interface DZOpenWPShopVC ()<UIImagePickerControllerDelegate,UIActionSheetDelegate,UINavigationControllerDelegate, DZCitySelectionVCDelegate>
...
- (IBAction)submitButtonAction {
    ...
    if (!self.addressTextField.text.length) {
        [SVProgressHUD showErrorWithStatus:@"请输入您的详细地址~"];
        return;
    }
    if (!self.categoryId) {
    ...
    NSDictionary *params = @{@"shop_type":@"3", @"shop_name":self.shopNameTextField.text, @"contacts_name":self.contactTextField.text, @"user_mobile":self.phoneTextField.text, @"province":self.province, @"city":self.city, @"address":self.addressTextField.text,@"category_id":self.categoryId, @"retail_amount":self.handCountTextField.text, @"basic_amount":self.expressCountTextField.text, @"allow_return":allow_return};
    ...
    NSURLSessionDataTask *task = [manager POST:[DEFAULT_HTTP_HOST stringByAppendingString:@"/index.php/Api/ShopEditApi/addShopStep1"] parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData> _Nonnull formData) {
