...
//        sureButton.sd_layout.leftSpaceToView(bgView, 0).rightSpaceToView(bgView, 0).bottomSpaceToView(bgView, 0).heightIs(49);
        ...
        [self.tableview mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.mas_equalTo(bgView);
            make.right.mas_equalTo(bgView);
            make.top.mas_equalTo(goodsInfo.mas_bottom);
...
-(void)initData:(GoodsModel *)model Andtype:(NSString *)str {
    ...
    [_dataSource addObjectsFromArray:model.itemsList];
    //此方法必须在_dataSource赋值后方可调用
    [self reloadGoodsInfos:str];
    [self.tableview reloadData];
}
- (void)reloadGoodsInfos:(NSString *)type {
    ...
    NSString *ids = [self getSizeId];
    self.typeIds = ids;
    
    [self getAttrRequests:type];
    NSLog(@"用户选择的属性是：%@  %@",str,ids);
}
- (void)getAttrRequests:(NSString *)type {
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    [params setObject:self.typeIds forKey:@"goods_spec_key"];
    [params setObject:goodsInfo.model.goodsId forKey:@"goods_id"];
    [params setObject:goodsInfo.model.act_Id forKey:@"act_id"];
    [params setObject:type forKey:@"goods_price"];
    LNetWorkAPI *api = [[LNetWorkAPI alloc] initWithUrl:@"/Api/IndexApi/getFreeTestPriceAndStoreBySpce" parameters:params method:LCRequestMethodPost];
    [api startWithBlockSuccess:^(__kindof LCBaseRequest *request) {
        NSString *str = request.rawJSONObject[@"data"];
        if ([str isKindOfClass:[NSString class]]) {
            [SVProgressHUD showInfoWithStatus:request.rawJSONObject[@"info"]];
            return;
        }
        if ([[request.responseJSONObject objectForKey:@"status"] integerValue] == 1) {
            self.storeStr = request.responseJSONObject[@"data"][@"store_count"];
            goodsInfo.goodsCountLabel.text = [NSString stringWithFormat:@"库存：%@",request.responseJSONObject[@"data"][@"store_count"]];
            goodsInfo.goodsPriceLabel.text = [NSString stringWithFormat:@"¥%@",request.responseJSONObject[@"data"][@"price"]];
        }else{
...
- (void)getAttrRequest{
    ...
    [params setObject:goodsInfo.model.goodsId forKey:@"goods_id"];
    [params setObject:goodsInfo.model.act_Id forKey:@"act_id"];
    LNetWorkAPI *api = [[LNetWorkAPI alloc] initWithUrl:@"/Api/CartApi/getPriceAndStoreBySpce" parameters:params method:LCRequestMethodPost];
    [api startWithBlockSuccess:^(__kindof LCBaseRequest *request) {
        NSString *str = request.rawJSONObject[@"data"];
        if ([str isKindOfClass:[NSString class]]) {
            [SVProgressHUD showInfoWithStatus:request.rawJSONObject[@"info"]];
...
//        if ([request.responseJSONObject[@"data"][@"store_count"] isKindOfClass:[NSNull class]]) {
//            self.storeStr = @"";
//        }else {
//        }
        if ([[request.responseJSONObject objectForKey:@"status"] integerValue] == 1) {
...
- (NSString *)getSizeId
{
    ...
    for (GoodsTypeModel *model in _dataSource) {
        if (model.selectIndex>=0) {
            if (str.length == 0) {
                str = model.typeIds[model.selectIndex];
            }else
                str = [NSString stringWithFormat:@"%@_%@",str,model.typeIds[model.selectIndex]];
        }
    }
    return str;
}
...
-(void)sure
{
    ...
    if (_dataSource.count == 0) {
     ...
     sizeModel = [[SizeAttributeModel alloc]init];
    //判断库存
    if ([self.storeStr intValue]>0) {
        if (self.selectSize) {
            if (countView.countTextField.text.length == 0) {
                sizeModel.count = @"1";
            }else{
                sizeModel.count = countView.countTextField.text;
            }
            NSLog(@">>>>>>>>===%@ %@",self.typeIds,sizeModel.count);
            self.selectSize(self.typeIds,sizeModel.count);
        }
        [self hideView];
    }else
    {
...
- (void)setIsdianji:(BOOL)isdianji {
    _isdianji = isdianji;
    if (isdianji == NO){
        countView.countTextField.enabled = NO;
    }
}
#pragma mark-数量加减
-(void)add
{
    ...
    if (sizeModel) {
        if (count <[self.storeStr intValue]) {
...
-(void)textFieldDidEndEditing:(UITextField *)textField
{
    ...
    if (sizeModel) {
        if (count >[self.storeStr intValue]) {
            ...
            countView.countTextField.text = self.storeStr;
        }
    }else
    {
        if (count > [self.storeStr intValue]) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.selectButton = ^(int selectIndex) {
        if (_iswoyaoshiyong.length != 0) {
            [weakSelf reloadGoodsInfos:_jiages];
        }else {
