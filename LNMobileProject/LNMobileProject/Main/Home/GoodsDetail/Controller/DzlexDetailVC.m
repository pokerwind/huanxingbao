...
@interface DzlexDetailVC ()<SDCycleScrollViewDelegate,UIScrollViewDelegate,UITableViewDelegate,UITableViewDataSource>
...
- (void)setupViews {
    if (self.iswodeshiyong.length == 0) {
        if ([self.model.goods_info.act_type integerValue] == 0) {
            ...
            [self.view addSubview:self.bottomV];
            [self.bottomV.btn setTitle:@"立即分享" forState:UIControlStateNormal];
            [self.bottomV mas_makeConstraints:^(MASConstraintMaker *make) {
                make.top.equalTo(self.scrollView.mas_bottom);
                make.left.bottom.right.mas_offset(0);
            }];
