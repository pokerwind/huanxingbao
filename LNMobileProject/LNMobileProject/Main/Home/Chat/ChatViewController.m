...
- (void)viewDidAppear:(BOOL)animated {
    [DPMobileApplication sharedInstance].isShowing = NO;
    [DPMobileApplication sharedInstance].hasNewChatMessage = NO;
                           ...
                           modelForMessage:(EMMessage *)message
{
    ...
    if (model.message.direction == 0) {
        ...
        model.avatarImage = [UIImage imageNamed:@"avatar_grey"];
        if ([user.head_pic hasPrefix:@"http"]) {
            model.avatarURLPath = user.head_pic;
        }else{
            model.avatarURLPath = [NSString stringWithFormat:@"%@%@",DEFAULT_HTTP_IMG,user.head_pic];
        }
        model.failImageName = @"imageDownloadFail";
        return model;
    } else {
        NSDictionary *dict = message.ext;
        if ([dict.allKeys containsObject:@"nick"]) {
            model.nickname = dict[@"nick"];
        }
        ...
        if ([dict.allKeys containsObject:@"avatar"]) {
            ...
            if ([avatar hasPrefix:@"http"]) {
                ...
                model.avatarURLPath = [NSString stringWithFormat:@"%@%@",DEFAULT_HTTP_IMG,avatar];
            }
        }
    ...
    if (model.message.chatType == 0) {
        ...
        if ([_imageurl hasPrefix:@"http"]) {
            ...
            headimage = [NSString stringWithFormat:@"%@%@",DEFAULT_HTTP_IMG,_imageurl];
        }
